...
The PostgreSQL connection settings are loaded from Azure App Configuration (see settings.py); the two
secrets among them are Key Vault references the provider resolves with the app's managed identity.
"""
import datetime
import hashlib
import logging
...
def make_activity_id(username: str, activity: str) -> str:
    """MD5 of username + activity + timestamp, preserving the source sample's ID scheme."""
    timestamp = datetime.datetime.now().isoformat()
    return hashlib.md5(f"{username}_{activity}_{timestamp}".encode()).hexdigest()


db_client: PostgresClient = PostgresClient.from_settings(load_settings())
