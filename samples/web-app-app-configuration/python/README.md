...
The load is retried for a few minutes, so a role assignment that has not propagated yet produces log lines rather than a crash loop, and the application logs which keys it loaded and how many Key Vault references it resolved, never the values. The settings are read once at startup; a changed key-value or secret takes effect when the app restarts. The app setting is called `Endpoints__AppConfiguration` in both language variants because it is the app-setting spelling of the `Endpoints:AppConfiguration` key the .NET configuration system reads.
