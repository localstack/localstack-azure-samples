locals {
  ...
  # Real Azure returns just the bare host on 5432. Split on ":" so the Web App always gets the
...
#   * pe-subnet      — hosts the Private Endpoint to the PostgreSQL flexible server (no
...
# Note: PG_USER and PG_PASSWORD are intentionally NOT set here. The post-apply step in
# deploy.sh connects to the server (via the firewall-allowed public endpoint) as the admin,
# creates the application role `testuser`, seeds the schema, and then writes `PG_USER` /
# `PG_PASSWORD` onto this Web App via `az webapp config appsettings set`. The server-admin
...
module "web_app" {
  ...
  app_settings = {
    ...
    PG_DATABASE                    = module.postgres_flexible_server.database_name
