...
@description('Specifies the FQDN of the PostgreSQL flexible server (e.g. <server>.postgres.database.azure.com).')
param pgHost string

@description('Specifies the TCP port the PostgreSQL server listens on. 5432 in real Azure; in the emulator the FQDN encodes the dynamically allocated proxy port and main.bicep splits it.')
param pgPort string = '5432'

@description('Specifies the name of the database to connect to.')
param pgDatabase string = 'sampledb'
...
resource configAppSettings 'Microsoft.Web/sites/config@2024-11-01' = {
  ...
  properties: {
    ...
    // PG_USER and PG_PASSWORD are NOT set here. The post-deploy script connects to the server
    // via the firewall-allowed public endpoint to (a) create the application role `testuser`
    // and (b) write `PG_USER` / `PG_PASSWORD` onto this Web App via `az webapp config
    // appsettings set`. The server-admin login is never exposed to the Web App at runtime.
    PG_HOST: pgHost
    PG_PORT: pgPort
