...
call_web_app() {
	...
	echo "Getting the host port mapped to internal port 8000 in container [$container_name]..."
	host_port=$(get_docker_container_port_mapping "$container_name" "8000")
	...
	if [ -n "$container_ip" ]; then
		...
		curl --max-time 10 -s "http://$container_ip:8000/" 1> /dev/null
