...
echo "=================================================="
echo "DEBUG: Starting bicep deployment for container-apps-blob-storage"
...
# (Bicep creates the container app referencing the image in ACR)
# =============================================================================

# Create ACR first so we can push the image
ACR_NAME="${PREFIX}acaacr${SUFFIX}"
...
if DEPLOYMENT_OUTPUTS=$(az deployment group create \
	...
	ENVIRONMENT_NAME=$(echo "$DEPLOYMENT_JSON" | jq -r '.environmentName.value')
	APP_NAME=$(echo "$DEPLOYMENT_JSON" | jq -r '.appName.value')
