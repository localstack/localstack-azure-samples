...
PRIVATE_ENDPOINT_GROUP="mysqlServer"
...
# Check if the MySQL flexible server already exists
echo "Checking if [$MYSQL_SERVER_NAME] MySQL flexible server already exists in the [$RESOURCE_GROUP_NAME] resource group..."
...
if [[ $? != 0 ]]; then
	echo "No [$MYSQL_SERVER_NAME] MySQL flexible server already exists in the [$RESOURCE_GROUP_NAME] resource group"
	echo "Creating [$MYSQL_SERVER_NAME] MySQL flexible server in the [$RESOURCE_GROUP_NAME] resource group..."

	# Create a MySQL flexible server with public network access
	...
	if [ $? -eq 0 ]; then
		echo "[$MYSQL_SERVER_NAME] MySQL flexible server successfully created in the [$RESOURCE_GROUP_NAME] resource group"
	else
		echo "Failed to create [$MYSQL_SERVER_NAME] MySQL flexible server in the [$RESOURCE_GROUP_NAME] resource group"
		exit 1
	fi
else
	echo "[$MYSQL_SERVER_NAME] MySQL flexible server already exists in the [$RESOURCE_GROUP_NAME] resource group"
fi

# Retrieve the resource id of the MySQL flexible server
echo "Getting [$MYSQL_SERVER_NAME] MySQL flexible server resource id in the [$RESOURCE_GROUP_NAME] resource group..."
...
if [ -n "$MYSQL_SERVER_ID" ]; then
	echo "MySQL flexible server resource id retrieved successfully: $MYSQL_SERVER_ID"
else
	echo "Failed to retrieve MySQL flexible server resource id."
	exit 1
fi

# Retrieve the fullyQualifiedDomainName of the MySQL flexible server
echo "Getting [$MYSQL_SERVER_NAME] MySQL flexible server FQDN in the [$RESOURCE_GROUP_NAME] resource group..."
...
if [ -n "$MYSQL_FQDN_FULL" ]; then
	echo "MySQL flexible server FQDN retrieved successfully: $MYSQL_FQDN_FULL"
else
	echo "Failed to retrieve MySQL flexible server FQDN."
	exit 1
fi
...
echo "Checking if [$FIREWALL_RULE_NAME] firewall rule already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server..."
...
if [[ $? != 0 ]]; then
	echo "No [$FIREWALL_RULE_NAME] firewall rule already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server"
	echo "Creating [$FIREWALL_RULE_NAME] firewall rule on the [$MYSQL_SERVER_NAME] MySQL flexible server..."

	# Create a permissive firewall rule so the deploy machine can run the mysql bootstrap.
	...
	if [ $FIREWALL_RULE_CREATED -eq 1 ]; then
		echo "[$FIREWALL_RULE_NAME] firewall rule successfully created on the [$MYSQL_SERVER_NAME] MySQL flexible server"
	else
		# Not fatal: the rule governs public network access, which the emulator does not enforce, and
		# the mysql bootstrap below fails loudly if the server is genuinely unreachable.
		echo "WARNING: could not create the [$FIREWALL_RULE_NAME] firewall rule on the [$MYSQL_SERVER_NAME] MySQL flexible server; continuing"
	fi
else
	echo "[$FIREWALL_RULE_NAME] firewall rule already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server"
fi

# Check if the MySQL database already exists
echo "Checking if [$MYSQL_DATABASE_NAME] database already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server..."
...
if [[ $? != 0 ]]; then
	echo "No [$MYSQL_DATABASE_NAME] database already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server"
	echo "Creating [$MYSQL_DATABASE_NAME] database on the [$MYSQL_SERVER_NAME] MySQL flexible server..."
	...
	if [ $? -eq 0 ]; then
		echo "[$MYSQL_DATABASE_NAME] database successfully created on the [$MYSQL_SERVER_NAME] MySQL flexible server"
	else
		echo "Failed to create [$MYSQL_DATABASE_NAME] database on the [$MYSQL_SERVER_NAME] MySQL flexible server"
		exit 1
	fi
else
	echo "[$MYSQL_DATABASE_NAME] database already exists on the [$MYSQL_SERVER_NAME] MySQL flexible server"
fi
...
if [[ -z $privateEndpointId ]]; then
	...
	echo "Creating [$PRIVATE_ENDPOINT_NAME] private endpoint for the [$MYSQL_SERVER_NAME] MySQL flexible server in the [$RESOURCE_GROUP_NAME] resource group..."

	# Create a private endpoint for the MySQL flexible server
	...
	if [[ $? == 0 ]]; then
		echo "Private endpoint [$PRIVATE_ENDPOINT_NAME] successfully created for the [$MYSQL_SERVER_NAME] MySQL flexible server in the [$RESOURCE_GROUP_NAME] resource group"
	else
		echo "Failed to create a private endpoint for the [$MYSQL_SERVER_NAME] MySQL flexible server in the [$RESOURCE_GROUP_NAME] resource group"
		exit
	fi
else
...
# Check if the private DNS zone group is already created for the MySQL flexible server private endpoint
...
if [[ -z $NAME ]]; then
	...
	# Create the private DNS zone group for the MySQL flexible server private endpoint
	az network private-endpoint dns-zone-group create \
		--name "$PRIVATE_DNS_ZONE_GROUP_NAME" \
		--resource-group "$RESOURCE_GROUP_NAME" \
		--endpoint-name "$PRIVATE_ENDPOINT_NAME" \
		--private-dns-zone "$PRIVATE_DNS_ZONE_NAME" \
		--zone-name "mysql-zone" \
		--only-show-errors 1>/dev/null
...
# Check whether the diagnostic settings for the MySQL flexible server already exist
echo "Checking if [$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server already exist..."
az monitor diagnostic-settings show \
	--name "$DIAGNOSTIC_SETTINGS_NAME" \
...
if [[ $? != 0 ]]; then
	echo "No [$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server actually exist"
	echo "Creating [$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server..."

	# Create the diagnostic settings for the MySQL flexible server to send logs to the Log Analytics workspace
	az monitor diagnostic-settings create \
		--name "$DIAGNOSTIC_SETTINGS_NAME" \
		--resource "$MYSQL_SERVER_ID" \
		--workspace "$LOG_ANALYTICS_NAME" \
	...
	if [[ $? == 0 ]]; then
		echo "[$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server successfully created"
	else
		echo "Failed to create [$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server"
		exit 1
	fi
else
	echo "[$DIAGNOSTIC_SETTINGS_NAME] diagnostic settings for the [$MYSQL_SERVER_NAME] MySQL flexible server already exist"
fi
