...
variable "input_queue_name" {
  ...
  validation {
    condition     = can(regex("^[a-z0-9]([a-z0-9-]*[a-z0-9])?$", var.input_queue_name)) && length(var.input_queue_name) >= 3 && length(var.input_queue_name) <= 63
    error_message = "Queue name must be lowercase alphanumeric characters or hyphens, between 3-63 characters."
  }
}
...
variable "output_queue_name" {
  ...
  validation {
    condition     = can(regex("^[a-z0-9]([a-z0-9-]*[a-z0-9])?$", var.output_queue_name)) && length(var.output_queue_name) >= 3 && length(var.output_queue_name) <= 63
    error_message = "Queue name must be lowercase alphanumeric characters or hyphens, between 3-63 characters."
  }
}

variable "trigger_queue_name" {
  description = "(Optional) Specifies the name of the trigger queue."
  type        = string
  default     = "trigger"

  validation {
    condition     = can(regex("^[a-z0-9]([a-z0-9-]*[a-z0-9])?$", var.trigger_queue_name)) && length(var.trigger_queue_name) >= 3 && length(var.trigger_queue_name) <= 63
    error_message = "Queue name must be lowercase alphanumeric characters or hyphens, between 3-63 characters."
  }
}
...
variable "input_table_name" {
  ...
  validation {
    ...
    error_message = "Table name must start with a letter, contain only alphanumeric characters, and be between 3-63 characters."
  }
}
...
variable "player_names" {
  description = "(Optional) Specifies the comma-separated list of player names."
  type        = string
  default     = "Alice,Anastasia,Paolo,Leo,Mia"

  validation {
    condition     = can(regex("^[a-zA-Z]+(,[a-zA-Z]+)*$", var.player_names))
    error_message = "Player names must be comma-separated alphabetic strings with no spaces."
  }
}

variable "timer_schedule" {
  description = "(Optional) Specifies the timer schedule for the timer triggered function."
  type        = string
  default     = "0 */1 * * * *"

  validation {
    condition     = can(regex("^[\\d\\s\\*\\/\\-,]+$", var.timer_schedule))
    error_message = "Timer schedule must be a valid cron expression."
  }
}
