locals {
  ...
  storage_account_name = "${var.prefix}urlshortstorage${var.suffix}"
  key_vault_name       = "${var.prefix}-urlshort-kv-${var.suffix}"
  postgres_server_name = "${var.prefix}-urlshort-pgflex-${var.suffix}"
  servicebus_namespace = "${var.prefix}-urlshort-sb-ns-${var.suffix}"
  log_analytics_name   = "${var.prefix}-urlshort-log-analytics-${var.suffix}"
  app_service_plan     = "${var.prefix}-urlshort-app-service-plan-${var.suffix}"
  web_app_name         = "${var.prefix}-urlshort-webapp-${var.suffix}"
  function_app_name    = "${var.prefix}-urlshort-functionapp-${var.suffix}"
  identity_name        = "${var.prefix}-urlshort-identity-${var.suffix}"
}
...
locals {
  pg_fqdn_parts = split(":", azurerm_postgresql_flexible_server.main.fqdn)
...
resource "azurerm_key_vault_secret" "pg_conn" {
  name         = "pg-conn"
  value        = "host=${local.pg_host} port=${local.pg_port} dbname=${azurerm_postgresql_flexible_server_database.clicks.name} user=${azurerm_postgresql_flexible_server.main.administrator_login} password=${random_password.postgres.result}"
  key_vault_id = azurerm_key_vault.main.id
}
...
resource "azurerm_linux_web_app" "web" {
  ...
  app_settings = {
    ...
    AZURE_TABLES_ENDPOINT          = azurerm_storage_account.main.primary_table_endpoint
    ...
    SB_CONN        = azurerm_servicebus_namespace.main.default_primary_connection_string
...
resource "azurerm_linux_function_app" "worker" {
  ...
  app_settings = {
    ...
    # The Functions host parses AzureWebJobsStorage with the strict .NET storage
    # clients, which cannot parse the EndpointSuffix-with-port string the
    # provider would compose on the emulator - so the provider default is
    # overridden with an explicit-endpoints connection, mirroring the Bicep
    # variant and the eventhubs sibling (the provider respects the override).
    ...
    ServiceBusConnection__clientId                = azurerm_user_assigned_identity.main.client_id
    ...
    WEB_BASE_URL   = "http://${azurerm_linux_web_app.web.default_hostname}"
    INTERNAL_TOKEN = random_password.internal_token.result
  }
}
